...
const CHANNELS = [
  {
    ...
    action: SITE_CONTACT.telDisplay,
    buttonLabel: "電話で相談する",
...
const HOURS = [
  { label: "電話受付", value: SITE_CONTACT.hours.tel },
  { label: "LINE・フォーム", value: SITE_CONTACT.hours.lineForm },
  { label: "見積もり目安", value: SITE_CONTACT.hours.estimate },
] as const
...
export function ContactChannels() {
  return (
    <section id="contact-channels" aria-labelledby="contact-channels-heading" className="bg-primary">
      <div className="mx-auto max-w-content px-4 py-14 md:px-6 md:py-24">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 leading-relaxed text-primary-foreground/70">
            <span className="font-bold text-primary-foreground">大体のサイズと業種さえ教えていただければ</span>
            、概算をすぐにお出しできます。
          ...
          {CHANNELS.map((ch) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="mt-4 text-xl font-bold text-primary-foreground">{ch.title}</h3>
                <p className="mt-3 min-h-[72px] text-sm leading-relaxed text-primary-foreground/65">{ch.description}</p>
                <p className="mt-4 text-base font-bold text-primary-foreground md:text-lg">{ch.action}</p>
                {ch.isExternal ? (
                  <a
                    href={ch.href}
                    target="_blank"
        ...
        <ul className="mt-10 grid gap-3 sm:grid-cols-3 sm:gap-4 md:mt-12">
          {HOURS.map((h) => (
            ...
            >
              <div className="text-xs font-bold tracking-widest text-primary-foreground/50">{h.label}</div>
              <div className="mt-1 text-sm font-bold text-primary-foreground/85">{h.value}</div>
            </li>
          ))}
        </ul>
      </div>
