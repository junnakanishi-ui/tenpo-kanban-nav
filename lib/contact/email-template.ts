...
export function buildAdminEmailText(values: ContactFormValues): string {
  ...
  if (values.simulatorData) {
    ...
    if (sim.thickness) lines.push(`厚み: ${sim.thickness}`)
    if (sim.mounting) lines.push(`加工: ${sim.mounting}`)
    if (sim.size) lines.push(`文字サイズ: ${sim.size}`)
    if (sim.spacer) lines.push(`スペーサー: ${sim.spacer}`)
    if (sim.bevel) lines.push(`面取り: ${sim.bevel}`)
    if (sim.mount) lines.push(`取付金具: ${sim.mount}`)
    if (sim.finish) lines.push(`仕上げ: ${sim.finish}`)
    if (sim.charHeight) lines.push(`文字高: ${sim.charHeight}mm`)
    if (sim.charCount) lines.push(`文字数: ${sim.charCount}文字`)
